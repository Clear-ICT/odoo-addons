...
To use it:
    1. Create a sequence type for the type of sequence(s) you want (in
       an xml data file)
    2. Override the partner create() method to create a sequence record for
       the sequence type created in step 1.  *Note*: this means that if
       you create N sequence types you will create N sequences every time
       a new partner is created.
    3. To get the next sequence call:
       self.env['ir.sequence'].get_next_by_partner(partner_id,
                                                   sequence_type_code)
    """,
